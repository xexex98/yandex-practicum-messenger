      ...
      events: {
        submit: async (e) => {
          ...
          if (!file) {
            ...
            const chatId = String(store.getState().chatId);

            form.append("chatId", String(chatId));
            await controller.updateChatAvatar(form);
...
export default connect(({ avatarError, loading, chatId }) => ({ avatarError, loading, chatId }))(
  LoadFile
);
